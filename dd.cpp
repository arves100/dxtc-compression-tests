...
HMODULE				g_DLL = NULL;			// ddraw.dll
/*--------------------------------------------------------------------------*/
typedef HRESULT(WINAPI* DirectDrawCreateCallback)(_In_opt_ GUID*, _Out_ LPDIRECTDRAW*, _In_opt_ IUnknown*);
/*--------------------------------------------------------------------------*/
...
bool DDInit( HWND hWnd )
{
	...
	g_DLL = LoadLibrary(_T("DDRAW.DLL"));
	if (!g_DLL)
		return false;

	auto cb = (DirectDrawCreateCallback)GetProcAddress(g_DLL, "DirectDrawCreate");
	if (!cb)
		return false;

	// Initialize DirectDraw
	hr = cb( NULL, &g_pDD, NULL );
	if (DDFailedCheck(hr, "DirectDrawCreate failed" ))
		return false;

	hr = g_pDD->QueryInterface(IID_IDirectDraw7, (LPVOID*)&g_pDD7);
	if (FAILED(hr))
		return false;
...
bool DDCreateSurfaces( bool bFullScreen, int width, int height, int bpp)
{
	...
	if (g_bFullScreen)
	{
		...
		hr = g_pDD7->SetCooperativeLevel(g_hWnd, DDSCL_EXCLUSIVE|DDSCL_FULLSCREEN|DDSCL_NOWINDOWCHANGES);
		if (DDFailedCheck(hr, "SetCooperativeLevel"))
			return false;
		
		// Set 640x480 full-screen mode
		hr = g_pDD7->SetDisplayMode(width, height, bpp, 0, 0);
	...
	else
	{
		...
		hr = g_pDD7->SetCooperativeLevel(g_hWnd, DDSCL_NORMAL);
	...
	else
	{
		...
		hr = g_pDD7->CreateSurface(&ddsd, &g_pDDS, NULL);
		...
		hr = g_pDD7->CreateSurface(&ddsd, &g_pDDSBack, NULL);
	...
	if (!g_bFullScreen)
	{
		...
		hr = g_pDD7->CreateClipper(0, &g_pClipper, NULL);
...
/*--------------------------------------------------------------------------*/
bool DDBlt(LPDIRECTDRAWSURFACE7 pSrc, int srcX, int srcY, int srcW, int srcH,
	LPDIRECTDRAWSURFACE7 pDst, int dstX, int dstY, int dstW, int dstH)
{
	RECT rcSrc, rcDst;
	rcSrc.top = srcY;
	...
	HRESULT hr = pDst->Blt(&rcDst, pSrc, &rcSrc, DDBLT_WAIT, NULL);
	return !DDFailedCheck(hr, "Blt failure");
}
/*--------------------------------------------------------------------------*/
LPDIRECTDRAWSURFACE7 DDCreateDXTSurface(int w, int h, int fcc)
{
	if ((w % 4) != 0)
		w += 4 - (w % 4);
	if ((h % 4) != 0)
	...
	kDDSDComp.dwFlags = DDSD_CAPS | DDSD_WIDTH | DDSD_HEIGHT | DDSD_PIXELFORMAT;
	kDDSDComp.dwWidth = w;
	kDDSDComp.dwHeight = h;
	...
	kDDSDComp.ddpfPixelFormat.dwFourCC = fcc;

	// Create the compressed surface of the correct size.
	LPDIRECTDRAWSURFACE7 pkCompSurf = NULL;
	auto hr = g_pDD7->CreateSurface(&kDDSDComp, &pkCompSurf, NULL);
